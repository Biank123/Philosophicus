...
//     try {
//       const response = await fetch(`http://localhost:3001/uploads/${file}`, {
//         headers: {
//           'Authorization': `Bearer ${localStorage.getItem('token')}`,
//         },
//       });
...
//         return URL.createObjectURL(blob); // Convertir a URL de objeto
//       } else {
//         console.error('Error al cargar la imagen:', response.status);
//         return null;
//       }
//     } catch (error) {
//       console.error('Error en la llamada de la imagen:', error);
//       return null;
//     }
//   };

//   // Este estado se moverá fuera del map para que sea común para todos los posts
//   const [imageSrcs, setImageSrcs] = useState({});

//   // Cargar imágenes para cada post al inicio
//   useEffect(() => {
//     const loadImages = async () => {
//       const srcs = {};
//       for (const post of posts) {
//         const imageUrl = await fetchImage(post.file); // Cambiar filename por post.file
//         srcs[post.id] = imageUrl;
//       }
//       setImageSrcs(srcs);
//     };
...
//         setComments((prev) => ({
//           ...prev,
//           [selectedPostId]: [...(Array.isArray(prev[selectedPostId]) ? prev[selectedPostId] : []), newCommentData],
//         }));
//         setNewComment('');
//       } else {
...
//             {post.file && isAuthenticated && (
//               <div>
//                 {/\.(jpeg|jpg|png)$/i.test(post.file) && (
//                   imageSrcs[post.id] && ( // Usar el estado de imagen
//                     <img
//                       src={imageSrcs[post.id]} // Usar la URL de la imagen cargada
//                       alt={post.title}
